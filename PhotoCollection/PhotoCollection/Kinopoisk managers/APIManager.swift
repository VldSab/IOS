...
extension APIManager {
    func JSONTaskWith(request: URLRequest, completion: @escaping JSONCompletionHandler) -> JSONTask {
        let dataTask = session.dataTask(with: request) { data, response, error in
            guard let response = response as? HTTPURLResponse else {
                ...
                let error = NSError(domain: KinopoiskNetworkingErrorDomain, code: MissingHTTPResponseError, userInfo: userInfo)
    ...
    func fetchWithArray<T: JSONDecodable>(request: URLRequest, parse: @escaping ([String: AnyObject]) -> [T]?, completion: @escaping (Result<[T], Error>) -> Void) {               
        let dataTask = JSONTaskWith(request: request) { json, response, error in
            DispatchQueue.main.async {
                if let error = error {
                    completion(.failure(error))
